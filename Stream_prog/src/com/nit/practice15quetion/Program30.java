...
public class Program30 {
	...
	public static void main(String[] args) {
		...
		List<Integer> collect2 = Arrays.stream(brr1).boxed().collect(Collectors.toList());
		
		List<Integer> collect3 = Arrays.stream(brr2).boxed()
		.filter(v -> collect2.contains(v))
		...
		int[] arr2 = { 3, 4, 5, 6 };
		 ...
		 List<Integer> result = Stream.of(arr1, arr2)
	                ...
	                .filter(i ->
	                			Arrays.stream(arr1).anyMatch(j -> j == i) &&
	                        Arrays.stream(arr2).anyMatch(j -> j == i))
	                .distinct()
	                .collect(Collectors.toList());
